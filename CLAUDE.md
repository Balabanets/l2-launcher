...
### 2026-07-24 — безопасное применение версий из админки
- Перед обновлением, восстановлением и запуском лаунчер всегда заново загружает и проверяет подписанный manifest: опубликованная из админки версия или rollback видны без перезапуска лаунчера.
- Отмена загрузки больше не запускает удаления и применение настроек.
- Для zstd показывается реальный сжатый объём; новые файлы проверяются во временном файле и только затем заменяют рабочие.
- Локально сохраняется только служебный `applied-client-manifest.json` (без файлов клиента). При переходе/rollback удаляются managed/optional-файлы, отсутствующие в целевой версии.
- Пути файлов безопасно URL-кодируются, включая пробелы, `#` и Unicode. Полностью скачанный `.part` проверяется без ошибочного Range-запроса `416`.
